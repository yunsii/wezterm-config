...
- Every new or renamed hotkey MUST be registered in `wezterm-x/commands/manifest.json` in the same edit that adds the binding — including Lua-only `action_callback` shortcuts that are never palette-invocable (use `"palette": { "display_only": true }` for those). The id used in the manifest must match the id passed to the hotkey counter via `wezterm-x/lua/usage.lua` or the tmux `command-chord` `hotkey-usage-bump.sh` call. Missing entries show up as `(unregistered)` in `scripts/dev/hotkey-usage-report.sh` and stay invisible in the command palette, so treat that report as the audit signal.
