...
- *Why both `PreToolUse` and `PostToolUse` fire `resolved`.* PreToolUse fires right after the user approves a permission prompt but *before* the tool runs, so `waiting → running` flips as soon as the approve keystroke lands. Without it, the counter and tab badge sit on `waiting` for the full tool-execution window (seconds to tens of seconds for slow tools) until PostToolUse fires. PostToolUse stays wired because it is the only hook that catches Monitor-subscription wake-ups — a streamed event landing after a prior turn's `Stop` causes Claude's first tool call (auto-allowed, no user action) to fire PostToolUse, which flips `done → running` so the counter reflects that Claude is back in a turn. PreToolUse covers that too in principle, but Monitor wake-ups often short-circuit tool calls, so PostToolUse is the durable path. The double-firing is idempotent: after PreToolUse has already flipped the entry, PostToolUse sees `running` and short-circuits on the fast path (no lock, no OSC tick, no log line).
