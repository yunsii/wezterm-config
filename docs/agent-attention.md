...
Use this doc when you need anything about the agent-attention pipeline: shared state file, Claude / Codex hook install or upgrade, status transitions, rendering, the four keyboard entry points (`Alt+j` / `Alt+k` / `Alt+l` / `Alt+/`), focus-based auto-ack, or provider integration.
