...
Aggregate press counts — no event log — for every WezTerm keymap entry and the tmux command-chord actions. The counter is meant for "do I press this often enough to deserve a better key" decisions, not forensics. For **timed shareable playback** of presses + session launches, see [`workday-playback.md`](./workday-playback.md) (separate narrative shards; the usage bump script appends those as a side effect).
